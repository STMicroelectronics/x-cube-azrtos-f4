...
UINT MX_FileX_Init(VOID *memory_ptr)
{
  ...
  ret = tx_byte_allocate(byte_pool, (VOID **) &read_buffer, FLASH_WRITE_SIZE, TX_NO_WAIT);
...
VOID fx_thread_entry(ULONG thread_input)
{
  ...
  do {
    ...
    /* Read FLASH_WRITE_SIZE chunk from the exec into buffer.  */
    status =  fx_file_read(&fx_file, (VOID *) read_buffer, FLASH_WRITE_SIZE, &bytes_read);
    ...
    /* Program FLASH_WRITE_SIZE bytes into flash */
    flash_ret = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, (uint32_t) FlashAddress, *((uint32_t*) read_buffer));
    ...
    /* Increment the flash address by number of written bytes */
    FlashAddress = FlashAddress + FLASH_WRITE_SIZE;
